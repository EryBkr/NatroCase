    ...
    where TContext : DbContext
{
    ...
    protected async Task SetEntityAsDeletedAsync(TEntity entity, bool permanent, CancellationToken cancellationToken)
    {
        if (!permanent)
        {
    ...
    private async Task setEntityAsSoftDeletedAsync(IEntityTimestamps entity, CancellationToken cancellationToken)
    {
        ...
        foreach (INavigation? navigation in navigations)
        {
            ...
            if (navigation.IsCollection)
            {
                if (navValue == null)
                {
                    ...
                    navValue = await GetRelationLoaderQuery(query, navigationPropertyType: navigation.PropertyInfo.GetType()).ToListAsync(cancellationToken);
                    if (navValue == null)
                        continue;
                }
            ...
            else
            {
                ...
                await setEntityAsSoftDeletedAsync((IEntityTimestamps)navValue, cancellationToken);
            }
        }
    ...
    protected async Task SetEntityAsDeletedAsync(IEnumerable<TEntity> entities, bool permanent, CancellationToken cancellationToken)
    {
        foreach (TEntity entity in entities)
            await SetEntityAsDeletedAsync(entity, permanent, cancellationToken);
    }
